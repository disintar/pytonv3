...
async def main(loop):
        ...
        async def wrapper(*args, **kwargs):
            try:
                return web.json_response(
                    {"ok": True, "result": await asyncio.wait_for(func(*args, **kwargs), 100 if high_timeout else 5)},
                    headers=headers)
            except Exception as e:
                try:
                    traceback.print_exc()
                    return web.json_response(
                        {"ok": False, "code": e.status_code if hasattr(e, "status_code") else None, "error": str(e)},
                        headers=headers)
                except:
    ...
    async def getTransactions(request):
        ...
        return await tonlib.get_transactions(address, from_transaction_lt=lt, from_transaction_hash=tx_hash,
                                             to_transaction_lt=to_lt, limit=limit)
    ...
    async def getBlockTransactions(request):
        ...
        return await tonlib.getBlockTransactions(workchain, shard, seqno, root_hash, file_hash, count, after_lt,
                                                 after_hash)
